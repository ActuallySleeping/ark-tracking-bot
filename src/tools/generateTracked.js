...
const getServerName = async (ip,port)=>{
	...
	await query.info(ip,port,500).then(info=>{
		if(info.map==null && info.game==null){
			db.all(`SELECT * FROM InformationServer WHERE ip=? AND port=?`,[ip,port], (err,rows)=>{
			...
			db.run(`REPLACE INTO InformationServer (ip,port,name,map,game) VALUES (?,?,?,?,?)`,[ip,port,rv.removeVersion(info.name),info.map,info.game])
...
const generateTracked = async (timer,client,guildid,channelid) =>{
	let db = new sqlite3.Database(baselocation)
	await db.all(`SELECT * FROM InformationPlayer WHERE guildid=? and channelid=?`,[guildid,channelid], async (err,rows)=>{
		if(rows.length>0 && rows.length!=undefined){
			for (let i in rows){
				...
				if(online==1){
					...
					db.run(`REPLACE INTO InformationPlayer (guildid,channelid,name,ip,port,laststate) VALUES(?,?,?,?,?,?)`,[rows[i].guildid,rows[i].channelid,rows[i].name,rows[i].ip,rows[i].port,1])
					}
				}
				else if(online==0){
					if(rows[i].laststate==1){
					channel.send(`${rows[i].name} disconnect from ${await getServerName(rows[i].ip,rows[i].port)}`)
					db.run(`REPLACE INTO InformationPlayer (guildid,channelid,name,ip,port,laststate) VALUES(?,?,?,?,?,?)`,[rows[i].guildid,rows[i].channelid,rows[i].name,rows[i].ip,rows[i].port,0],(err)=>{return})
					}
				}
			}
		}
